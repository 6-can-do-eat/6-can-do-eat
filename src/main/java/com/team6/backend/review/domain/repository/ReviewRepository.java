...
public interface ReviewRepository extends JpaRepository<Review, UUID> {
    ...
    Page<Review> findByStore_StoreIdAndDeletedAtIsNull(UUID storeId, Pageable pageable);

    @Query("SELECT COALESCE(AVG(r.rating), 0.0) FROM Review r WHERE r.store.storeId = :storeId AND r.deletedAt IS NULL")
